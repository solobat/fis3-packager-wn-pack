...
var confReleaseDir = fis.media().get('release.jsPkgDir');
var RELEASE_DIR = typeof confReleaseDir === 'undefined'? 'js' : confReleaseDir;
...
function getAmdConfig() {
    var amdConfig = fis.config.get('amd') || {};
    var requireConfig = {};
    ['baseUrl', 'paths', 'packages', 'shim'].forEach(function (key) {
        amdConfig[key] && (requireConfig[key] = amdConfig[key]);
    });
...
function Context(ret) {
    ...
    this.requireConfig = getAmdConfig();
...
Context.prototype.resolvePath = function (url, file) {
    if (util.isLocalPath(url) && file) {
        ...
        url = amdHelper.join(amdHelper.dirname(file.url), url);
    }
    return url;
};
...
Context.prototype.getModuleId = function (file) {
    ...
    if (file.isJsLike) {
        id = file.moduleId || file.extras.moduleId || amdHelper.getModuleId(fullPath, this.moduleConfig, true);
    ...
    pkgs.forEach(function (item) {
        ...
        if (item.isJsLike) {
            for (var i = includes.length - 1; i >= 0; i--) {
                ...
                var packBundlesTo = file.packBundlesTo;
                var packFile = item;
                if(packBundlesTo){
                    packFile = this.getFileBySubPath(packBundlesTo);
                }
                pathMap[this.getModuleId(file)] = this.getReleaseModuleId(packFile);
            }
        } else {
            // FIXME: 如果存在同路径但后缀不一样的资源文件，会导致 pathmap 有问题。。
