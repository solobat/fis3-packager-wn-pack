...
function parsePackScript(file, context, opts) {
    ...
    var getPlaceholder = function () {
        ...
        // 在requirejs之后或者在异步入口模块前插入 requrie.config 脚本
...
module.exports = exports = {
     ...
     * @return {string}
     */
    createScriptTags: function (host, scripts, attrs,outputAsynPkg) {
        ...
        outputAsynPkg || (outputAsynPkg = false);
        var result = [];
        scripts.forEach(function (file) {
            //如果是异步请求合成的文件，而且配置了不输出异步打包的文件到页面上，则不创建此script标签
            if(file.isAsynPkg && outputAsynPkg){
                return;
            }
