...
/**
 * 获取html页面所有的异步请求的模块（包括模块的依赖）
 *
 * @inner
 * @param {arrFile} 异步模块文件id组成的数组
 * @param {context} context对象
 * @return {Array} 所有依赖的异步模块的file对象组成的数组
 */
function getDepAsynScripts(arrFile,context){
    var existsMap = {};
    var output = [];
    while(arrFile.length){
        var file = arrFile.shift();
        if(existsMap[file]){
            continue;
        }
        var fileObj = context.fileIdMap[file];
        if(!fileObj){
            fis.log.warn('file[%s] is not in context.fileIdMap',file);
            continue;
        }
        output.push(fileObj);

        if(fileObj.requires){
            var arrRequires = fileObj.requires;
           if(!Array.isArray(arrRequires)){
                arrRequires = [arrRequires];
            }
            arrFile = arrFile.concat(arrRequires);
        }
    }
...
function getOutputResourcePathMap(packedFiles, context, parseOpts, settings) {
    ...
    pages.forEach(function(page) {
        ...
        //处理不打包情况下，生成该html页面依赖的异步模块的pathmap
